...
/*   Updated: 2022/08/29 15:05:50 by christopher      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_time_philo(t_philo *philo, int nb_philo, t_params *params)
{
	...
	if (start_time == -1)
		return (1);
	params->init_time = start_time + 1500;
	while (i < nb_philo)
	{
...
int	threadator(t_philo *philo, t_params *params)
{
	...
	if (!th_philo)
		return (1);
	th_shinigami = malloc(sizeof(pthread_t) * nb_philo);
	if (!th_shinigami)
		return (1);	
	set_time_philo(philo, nb_philo, params); //TODO secu
	if (create_shinigami(philo, th_shinigami, nb_philo))
		return (1);
	if (create_thread(philo, th_philo, nb_philo))
	{
		end_philo_thread(nb_philo, th_shinigami);
		return (1);
	}
	end_philo_thread(nb_philo, th_shinigami);
